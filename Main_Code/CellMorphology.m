function [CellState, Ctgfb, Dtgfb, Cstate] = CellMorphology(CellState, Param, Ctgfb, Dtgfb)
% CellMorphology.m: Modifies the starting cell structure.

MorphType           = Param.Morphology;
CultureModel        = Param.CellTypes;
FibroblastCount     = Param.FibroblastCount;
EMTThresh           = Param.EMTThresh;
...
switch MorphType
            ...
            for j = 1:n
                for k = 1:n
                    if ((i-h)^2 + (j-h)^2 + (k-h)^2 <= Param.Crado^2) && ((i-h)^2 + (j-h)^2 + (k-h)^2 >= Param.Cradi^2)
                       ...
                       Dtgfb(i,j,k)          = Param.Dcell * (1- (EMTThresh*CellState.p(cnt))); % defining diffusion coefficient for each cell
            ...
            for j = 1:n
                for k = 1:n
                    if ((j-h)^2 + (k-h)^2 <= Param.Crado^2) && ((j-h)^2 + (k-h)^2 >= Param.Cradi^2) % ((i-h)^2 + (j-h)^2 + (k-h)^2 <= Crado^2) && ((i-h)^2 + (j-h)^2 + (k-h)^2 >= Cradi^2)
                       ...
                       CellState.p(cnt)      = CellState.conc(cnt, 8) / Param.NcadMax; % probability threshold dependent on Ncad
                       Dtgfb(i,j,k)          = Param.Dcell * (1- (EMThresh*CellState.p(cnt))); % defining diffusion coefficient for each cell
                       CellState.Dcell(cnt)  = Dtgfb(i,j,k); 
                       CellState.Ctgfb(cnt)  = Ctgfb(i,j,k); % TGFB concentration gradient
    ...
    case 3
        ...
        for i = 1:length(curvet)
           ...
           CellState.p(cnt)      = CellState.conc(cnt, 8) / Param.NcadMax; % probability threshold dependent on Ncad
           Dtgfb(curvet(i, 1), curvet(i, 2), curvet(i, 3)) = Param.Dcell * (1- (EMThresh*CellState.p(cnt))); % defining diffusion coefficient for each cell
...
if contains(CultureModel, 'Epithelial Only')
    ...
    for v = 1:FibroblastCount  
        ...
        if ((pos(1)-h)^2 + (pos(2)-h)^2 + (pos(3)-h)^2 > Param.Crado^2)
           ...
           Dtgfb(pos(1), pos(2), pos(3)) = Param.Dcell * (1- (EMTThresh*CellState.p(cnt)));
